...
public class ReleaseTalkActivity extends BaseActivity {
    ...
    private int count = 0, dynamicId = 0, userId, countIndex = 0;
    ...
    private void login(int count) {
        loginData.userLogin(ReleaseTalkActivity.this, GenerateUtils.generatePhone(count), new LoginData.LoginImp() {
            @Override
    ...
    private void getTokenTalk() {
        if (images==null || TextUtils.isEmpty(String.valueOf(userId))) return;
            //获取token
            getMultiToken.getMultiToken(ReleaseTalkActivity.this, UploadUtils.FILE_TYPE_TALK, new GetMultiToken.MultiTokenImp() {
                @Override
    ...
    private void upTalkqiniu() {
        ...
        //上传七牛
        upLoadDynamic.uploadTalk(userId, imgPath, "这只小猫猫在观赏花园", keyType, listBaseEntity, new UpLoadDynamic.UpLoadQiNiuTalkImp() {
            @Override
    ...
    private void insertTalk(final TalkRequestModel mode) {
        upLoadDynamic.releaseTalk(ReleaseTalkActivity.this, mode, new UpLoadDynamic.ReleaseSuccessImp() {
            @Override
            public void onReleaseSuccess(boolean b, final int dynamicsId) {
                dynamicId = dynamicsId;
                if (b) {
                    getGold(dynamicId, "talk");
                } else {
                    deleteTalk(dynamicId);
                }
            }
        });
    }

    private void getGold(final int dynamicId, String earnType) {
        if (dynamicId == 0 || userId == 0) return;
        upLoadDynamic.updateActivityEarnTwice(ReleaseTalkActivity.this, dynamicId, userId, earnType, new UpLoadDynamic.UpdateActivityEarnTwiceImp() {
            @Override
    ...
    private void deleteTalk(int dynamicId) {
        if (dynamicId == 0) return;
        upLoadDynamic.deleteDynamicDetails(ReleaseTalkActivity.this, dynamicId, new UpLoadDynamic.onDeleteTalkImp() {
            @Override
